...
!    vbot_idx       = pbuf_get_index('V10')
  ...
  subroutine cospsimulator_intr_run(state,pbuf, cam_in,emis,coszrs,cld_swtau_in,snow_tau_in,snow_emis_in)    
    ...
    real(r8) :: mr_lsliq(pcols,pver)                     ! mixing_ratio_large_scale_cloud_liquid (kg/kg) (gricell avg.)
    ...
    real(r8) :: icimr(pcols,pver)                        ! In cloud ice mixing ratio, JKS
    real(r8) :: icwmr(pcols,pver)                        ! In cloud water mixing ratio, JKS
    ...
    ! NOTE: Mass mixing ratios are over dry air!
...
!    real(r8), pointer, dimension(:)   :: vbot       ! Lowest model level meridional wind
    ...
    ! JKS - No radiatively active CO or SO2 in RRTMG or at least in CESM2.
    ...
    icimr(1:ncol,1:pver)              = 0._r8 ! JKS
    icwmr(1:ncol,1:pver)              = 0._r8 ! JKS
    ...
    ! Calculate cloud liquid water and ice consistently with the radiation scheme.
    ...
    ! Calculate in-cloud liquid water and ice mixing ratios (most consistent with the radation scheme)
    do k = top_lev, pver
      do i = 1, ncol
    ...
    cospstateIN%at                             = state%t(1:ncol,1:pver) ! Layer temperatures.
...
! For winds take the total 10m wind from cam_in and divide it such that the quadrature sum is the same.
    ...
    cospstateIN%phalf(1:ncol,1)                = 0._r8 ! JKS why???? r_state indicates that the interface above the top layer is 2.25mb.
...
!    end if
   ...
   !  end where
    ...
    ! ^This field is filled with 50um values when no cloud is present. Should I worry about this? 
    ! ^I could set dei to zero and let the default RTTOV scheme handle it or just stick with the CESM filler value.
          
    ! RTTOV doesn't consider precip flux, but if I add RTTOV-SCATT it will.
...
!    end if 
  ...
  subroutine construct_cospstateIN(npoints,nlevels,y)
    ...
    allocate(y%sunlit(npoints),y%skt(npoints),y%land(npoints),y%at(npoints,nlevels),     &
             ...
             y%co2(npoints,nlevels),y%psfc(npoints), &
